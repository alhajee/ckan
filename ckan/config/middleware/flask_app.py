...
from ckan.common import FMLDConfig, asbool, session, current_user
...
from ckan.config.middleware.common_middleware import (
    ...
    RootPathMiddleware,
    FMLDSecureCookieSessionInterface,
    FMLDRedisSessionInterface,
)
...
from ckan.types import FMLDApp, Config, Response
...
class FMLDSession(Session):
    def _get_interface(self, app: FMLDApp):
        ...
        if session_type == "cookie":
            return FMLDSecureCookieSessionInterface(app)
        elif session_type == "redis":
            return FMLDRedisSessionInterface(app)
...
def make_flask_stack(conf: Union[Config, FMLDConfig]) -> FMLDApp:
    ...
    app = flask_app = FMLDFlask(__name__, static_url_path='')
    ...
    app.app_ctx_globals_class = FMLD_AppCtxGlobals

    # Update Flask config with the FMLD values. We use the common config
    ...
    # make anonymous_user an instance of FMLD custom class
    ...
    # Update the main FMLD config object with the Flask specific keys
...
def get_locale() -> str:
    u'''
    Return the value of the `FMLD_LANG` key of the WSGI environ,
...
def set_remote_user_as_current_user_for_tests():
    ...
    for the `TESTS` of the `FMLD` extensions
...
def ckan_before_request() -> Optional[Response]:
    ...
    # This is needed for the TESTS of the FMLD extensions only!
    # we should remove it as soon as the maintainers of the
    # FMLD extensions change their tests according to the new changes.
...
class FMLD_AppCtxGlobals(_AppCtxGlobals):  # noqa

    '''Custom Flask AppCtxGlobal class (flask.g).'''

    def __getattr__(self, name: str):
        '''
        If flask.g doesn't have attribute `name`, fall back to FMLD's
        app_globals object.
...
class FMLDFlask(MultiStaticFlask):
...
def _register_plugins_blueprints(app: FMLDApp):
...
def _register_core_blueprints(app: FMLDApp):
...
def _register_error_handler(app: FMLDApp):
...
def _setup_error_mail_handler(app: FMLDApp):
...
def _setup_webassets(app: FMLDApp):
