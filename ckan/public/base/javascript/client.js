  ...
  jQuery.extend(Client.prototype, {
    ...
    parseCompletions: function (data, options) {
      ...
      var items = jQuery.map(raw, function (item) {
        ...
        var label =
          typeof options.label != "undefined" ? item[options.label] : false;
    ...
    parsePackageCompletions: function (string, options) {
      ...
      return jQuery.map(packages, function (pkg) {
        ...
        return options && options.objects === true
          ? { id: id, text: text }
          : id;
      });
    },
    ...
    getStorageMetadata: function (key, success, error) {
      if (!key) {
        throw new Error(
          "Client#getStorageMetadata() must be called with a key"
        );
      }
    ...
    convertStorageMetadataToResource: function (meta) {
      ...
      var createdISO = jQuery.date.toFMLDString(created);
      var modifiedISO = jQuery.date.toFMLDString(modified);
