...
function filterInfo(
  dt,
  noHtml = false,
  justFilterInfo = false,
  wrapped = false
) {
  let filtermsg = justFilterInfo
    ? ""
    : document.getElementById("dtprv_info").innerText;
  ...
  if (gsearchMode === "table") {
    filtermsg =
      filtermsg + "<br/> <b>" + gtableSearchText + ":</b> " + dt.search();
    ...
    dt.columns().every(function () {
      ...
      if (colsearch) {
        colsearchflag = true;
        colsearchmsg =
          colsearchmsg + " <b>" + colname + ":</b><br/>" + colsearch + ", ";
      }
    });
    if (colsearchflag) {
      filtermsg =
        ...
        gcolFilterText +
        ": <br/></b>" +
        colsearchmsg.slice(0, -2);
    }
  }
  filtermsg = justFilterInfo ? filtermsg : filtermsg + "<br/>" + gsortInfo;
  filtermsg = noHtml ? filtermsg.replace(/(<([^>]+)>)/gi, "") : filtermsg;
...
function copyLink(dt, deeplink, shareText, sharemsgText) {
  ...
  const textarea = $("<textarea readonly/>").val(deeplink).appendTo(hiddenDiv);
...
function initFilterObserver() {
  ...
  try {
    filterObserver.observe(document.getElementById("dtprv_info"), {
      characterData: true,
      subtree: true,
      childList: true,
    });
...
const linkify = (input) => {
  ...
  if (linksFound != null) {
    if (
      linksFound.length === 1 &&
      input.match(/\.(jpeg|jpg|gif|png|svg|apng|webp|avif)$/)
    ) {
      // the whole text is just one link and its a picture, create a thumbnail
      text =
        '<div class="thumbnail zoomthumb"><a href="' +
        linksFound[0] +
    ...
    initialize: function () {
      ...
      gdataDict.forEach((colDefn, idx) => {
        ...
        switch (colDefn.type) {
          ...
          case "timestamptz":
            colDict.type = "date";
            if (formatdateflag) {
              colDict.render = $.fn.dataTable.render.moment(
          ...
          default:
            ...
            colDict.render = function (data, type, row, meta) {
              ...
              if (!linkifiedData.links) {
                ...
                // there are links
                const strippedData = linkifiedData.text.replace(
                  /(<([^>]+)>)/gi,
                  ""
                );
                ...
                for (const aLink of linkifiedData.links) {
                  ...
                  if (linkpos + aLink.length >= ellipsislength) {
                    ...
                    addLen =
                      addLen + lastlink.length ? lastlink.length + 31 : 0; // 31 is the number of other chars in the full anchor tag
                    lastpos = linkpos;
                    lastlink = aLink;
                  }
                }
                const shortened = linkifiedData.text
                  .substr(0, lastpos + addLen)
                  .trimEnd();
                return (
                  '<span class="ellipsis" title="' +
                  esc(strippedData) +
          ...
          details: {
            display: $.fn.dataTable.Responsive.display.modal({
              header: function (row) {
                ...
                var data = row.data();
                return (
                  '<span style="font-size:150%;font-weight:bold;">Details:</span>&nbsp;&nbsp;<div class=" dt-buttons btn-group">' +
                  '<button id="modalcopy-button" class="btn btn-default" title="' +
                  that._("Copy to clipboard") +
                  '" onclick="copyModal(\'' +
                  packagename +
                  "&mdash;" +
                  resourcename +
                  '\')"><i class="fa fa-copy"></i></button>' +
                  '<button id="modalprint-button" class="btn btn-default" title="' +
                  that._("Print") +
                  '" onclick="printModal(\'' +
                  packagename +
                  "&mdash;" +
            ...
            renderer: function (api, rowIdx, columns) {
              const data = $.map(columns, function (col, i) {
                return col.className !== " none"
                  ? '<tr class="dt-body-right" data-dt-row="' +
                      col.rowIndex +
                      '" data-dt-column="' +
                      col.columnIndex +
                      '">' +
                      ...
                      ":" +
                      "</td><td>" +
                      (col.data.startsWith('<span class="ellipsis"')
                        ? col.data.substr(30, col.data.indexOf('">') - 30)
                        : col.data) +
                      "</td></tr>"
                  : "";
              }).join("");
              return data
                ? $('<table class="dtr-details" width="100%"/>').append(data)
                : false;
            },
          },
        };
      ...
      $(".fhead").each(function (i) {
        ...
        const placeholderText =
          formatdateflag && coltype.substr(0, 9) === "timestamp"
        ...
        $(
          ...
          .on("keyup search", function (event) {
            ...
            if (
              event.keyCode === 13 ||
              (this.value === "" &&
                datatable.column(colSelector).search() !== "")
            ) {
              datatable
                .column(colSelector)
                .search(this.value)
      ...
      datatable = $("#dtprv").DataTable({
        ...
        initComplete: function (settings, json) {
          ...
          $("#dtprv_filter label").before(
            '<i id="filterinfoicon" class="fa fa-info-circle"</i>&nbsp;'
          );

          // on mouseenter on Search info icon, update tooltip with filterinfo
          $("#filterinfoicon").mouseenter(function () {
            document.getElementById("filterinfoicon").title =
              filterInfo(datatable, true, true, true) +
              "\n" +
              (gelapsedTime / 1000).toFixed(2) +
              " " +
              that._("seconds") +
              "\n" +
          ...
          const resourceInfo =
            document.getElementById("dtv-resource-info").innerText;
          $("div.resourceinfo").html(
            '<a href="' +
              ...
              "&mdash;" +
              resourcename +
              '</a> <i class="fa fa-info-circle" title="' +
              resourceInfo +
              '"</i>'
          );
          ...
          tableSearchInput.bind("keyup search", function (event) {
            ...
            if (
              event.keyCode === 13 ||
              (tableSearchInput.val() === "" && datatable.search() !== "")
            ) {
          ...
          document.getElementsByClassName("dt-view")[0].style.visibility =
            "visible";
          ...
          if (state) {
            ...
            if (json.recordsTotal > currPageLen) {
              const scrollBodyHeight =
                $("#resize_wrapper").height() -
                $(".dataTables_scrollHead").height() * 2.75;
            ...
            postfixButtons: [
              {
                ...
                text:
                  '<i class="fa fa-undo"></i> ' + that._("Restore visibility"),
              },
              {
                extend: "colvisGroup",
          ...
          },
          {
            ...
            action: function (e, dt, node, config) {
              ...
              const sharelink =
                ...
                "?state=" +
                window.btoa(JSON.stringify(dt.state()));
              copyLink(
                dt,
                sharelink,
                that._("Share current view"),
                that._("Copied deeplink to clipboard")
              );
            },
          },
        ],
      ...
      // called after getting an AJAX response from FMLD
      ...
      datatable.on("order", function () {
        ...
        gsortInfo =
          "<b> " +
          that._("Sort") +
          '</b> <i id="sortinfoicon" class="fa fa-info-circle" title="' +
          that._(
...
function animateEl(element, animation, complete) {
  if (!(element instanceof jQuery) || !$(element).length || !animation)
    return null;
