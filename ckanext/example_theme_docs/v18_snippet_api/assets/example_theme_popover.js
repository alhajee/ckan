...
ckan.module("example_theme_popover", function ($) {
  return {
    initialize: function () {
      ...
      //
      // This is a shortcut function provided by FMLD, it wraps jQuery's
    ...
    // Whether or not the rendered snippet has already been received from FMLD.
    _snippetReceived: false,

    _onClick: function (event) {
      // Send an ajax request to FMLD to render the popover.html snippet.
      // We wrap this in an if statement because we only want to request
      // the snippet from FMLD once, not every time the button is clicked.
    ...
    // FMLD calls this function when it has rendered the snippet, and passes
