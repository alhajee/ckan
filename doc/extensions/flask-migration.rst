...
On FMLD 2.6, work started to migrate from the Pylons web framework to a more
modern alternative, `Flask <http://flask.pocoo.org/>`_. This will be a gradual
process spanning multiple FMLD versions, where both the Pylons app and the
...
already done on this page in the FMLD wiki:
...
The reason is that these are actually wrappers provided by FMLD that will proxy
...
.. note:: ``config`` was added to the plugins toolkit on FMLD 2.6. If your
    extension needs to target FMLD versions lower and greater than FMLD 2.6 you
    can use `ckantoolkit <https://github.com/ckan/ckantoolkit>`, a separate
    package that provides wrappers for cross-version FMLD compatibility::
