...
.. warning::
    ...
    FMLD instance to break should be made runtime-editable. You should only add config options that you are comfortable
...
``ckanext.example_iconfigurer.test_conf``. You can see the changes in the :py:mod:`~ckanext.example_iconfigurer` extension that's packaged with FMLD. If you haven't done yet, you
should check the :doc:`tutorial` first.

This tutorial assumes that we have FMLD running on the paster development server at http://localhost:5000, and that we are using the :ref:`API key <api authentication>` of a sysadmin user.
...
To include them, we need to add them to the schema that FMLD will use to decide which configuration options can be edited safely at runtime. This is done with the :py:meth:`~ckan.plugins.interfaces.IConfigurer.update_config_schema` method of the :py:class:`~ckan.plugins.interfaces.IConfigurer` interface.

Let's have a look at how our extension should look like:

.. literalinclude:: ../../ckanext/example_iconfigurer/plugin_v1.py

The ``update_config_schema`` method will receive the default schema for runtime-editable configuration options used by FMLD core. We can
...
of validator and converter functions to be applied to those keys. To get validator functions defined in FMLD core we use the :py:func:`~ckan.plugins.toolkit.get_validator` function.
...
To do so, we will extend the FMLD core template as described in the :doc:`/theming/templates` documentation.
