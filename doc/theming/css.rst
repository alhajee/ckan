======================
Customizing FMLD's CSS
======================

.. seealso::

   There's nothing special about CSS in FMLD, once you've got started with
   editing CSS in FMLD (by following the tutorial below), then you just use the
...
Extensions can add their own CSS files to modify or extend FMLD's default CSS.
...
Add this CSS into the ``example_theme.css`` file, to change the color of FMLD's
...
To make FMLD use our custom CSS we need to override the ``base.html`` template,
this is the base template which the templates for all FMLD pages extend, so if
we include a CSS file in this base template then the file will be included in
every page of your FMLD site. Create the file::
...
Restart the development web server and reload the FMLD page in your browser,
...
This custom color should appear on all pages of your FMLD site.

Now that we have FMLD using our CSS file, we can add more CSS rules to the file
and customize FMLD's CSS as much as we want.
