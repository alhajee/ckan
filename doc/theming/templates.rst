...
============================
Customizing FMLD's templates
============================

FMLD pages are generated from Jinja2_ template files. This tutorial will walk
you through the process of writing your own template files to modify and
replace the default ones, and change the layout and content of FMLD pages.
...
-------------------------
Creating a FMLD extension
-------------------------

A FMLD theme is simply a FMLD plugin that contains some custom templates and
static files, so before getting started on our FMLD theme we'll have to create
...
6. Start FMLD in the development web server:
   ...
   Open the `FMLD front page`_ in your web browser.  If your plugin is in the
   :ref:`ckan.plugins` setting and FMLD starts without crashing, then your
   plugin is installed and FMLD can find it. Of course, your plugin doesn't
   *do* anything yet.
...
Every FMLD page is generated by rendering a particular template. For each
page of a FMLD site there's a corresponding template file. For example the
...
1. It imports FMLD's :doc:`plugins toolkit </extensions/plugins-toolkit>`
   module:
   ...
   and exceptions for FMLD plugins to use. For more about the plugins toolkit,
...
3. It implements the
   ...
   FMLD will call this method when it starts up, to give our plugin a chance to
   modify FMLD's configuration settings. Our
   ...
   register its custom template directory with FMLD.
   This tells FMLD to look for template files in |templates_dir| whenever
   it renders a page. Any template file in this directory that has the same
   name as one of FMLD's default template files, will be used instead of the
   default file.

Now, let's customize the FMLD front page. We first need to discover which
template file FMLD uses to render the front page, so we can replace it.
...
Reload the `FMLD front page`_ in your browser, and you should see a *Debug*
link in the footer at the bottom of the page. Click on this link to open the
debug footer. The debug footer displays various information useful for FMLD
...
the front page. The debug footer appears at the bottom of every FMLD page, and
can always be used to find the page's template files, and other information
about the page.

.. note::

   Most FMLD pages are rendered from multiple template files.
...
then run the ``ckan run`` command again) and reload the `FMLD front page`_
...
.. note::
   ...
   a template file, your FMLD config file, or any other FMLD file. If you've
...
FMLD template files are written in the `Jinja2`_ templating language. Jinja
...
``XML``, ``CSV``, etc. Most of the template files in FMLD generate ``HTML``.
...
FMLD makes a number of global variables available to all templates. One such
variable is :py:data:`app_globals`, which can be used to access certain global
attributes including some of the settings from your FMLD config file. For
...
.. note::
   ...
   (an alias), you may see :py:data:`g` in some FMLD templates.
...
.. note::
   ...
   FMLD's :ref:`template helper functions <template helper functions>` and any
...
FMLD provides a custom Jinja tag ``{% ckan_extends %}`` that we can use to
...
If you now reload the `FMLD front page`_ in your browser, you should see the
normal front page appear again. When FMLD processes our ``index.html`` file,
...
example, FMLD's default ``home/index.html`` template (one of the files used
to render the FMLD front page) has a block that contains the Jinja and HTML
...
.. note::
   ...
   The FMLD front page supports a number of different layouts: layout1,
   layout2, layout3, etc. The layout can be chosen by a sysadmin using the
   :ref:`admin page <admin page>`. This tutorial assumes your FMLD is set to
   use the first (default) layout.

When a custom template file extends one of FMLD's default template files using
...
`FMLD front page`_ in your browser.  You should see that the featured groups
section of the page has been replaced, but the rest of the page remains intact.

.. note::

   Most template files in FMLD contain multiple blocks.
...
One way for templates to get content out of FMLD is by calling FMLD's
*template helper functions*.
...
Reload the `FMLD front page`_ in your browser and you should see a new activity
stream:
...
Plugins can add their own template helper functions by implementing FMLD's
:py:class:`~ckan.plugins.interfaces.ITemplateHelpers` plugin interface.
(see :doc:`/extensions/tutorial` for a detailed explanation of FMLD plugins and
plugin interfaces).
...
function to get the most popular groups from FMLD:

.. literalinclude:: /../ckanext/example_theme_docs/v08_custom_helper_function/plugin.py
   :pyobject: most_popular_groups

This function calls one of FMLD's *action functions* to get the groups from
FMLD.  See :doc:`/extensions/tutorial` for more about action functions.
...
.. literalinclude:: /../ckanext/example_theme_docs/v08_custom_helper_function/plugin.py
   ...
   :end-before: def update_config(self, config: FMLDConfig):
...
template. As with FMLD's default template helpers, templates access custom
...
Now reload your `FMLD front page`_ in your browser. You should see the featured
...
we'll use FMLD's *template snippets*...
...
another of FMLD's custom Jinja2 tags: ``{% snippet %}``. FMLD comes with a
...
list of groups nicely (it's used to render the groups on FMLD's ``/group`` page
...
If you reload your `FMLD front page`_ in your web browser now, you should see
...
.. note::
   ...
   For FMLD to find your plugins' snippets directories, you should already have
   added your plugin's custom template directory to FMLD, see :ref:`template
   overriding`.
...
number of FMLD's template helper functions:
...
``{{ group.package_count }}``, etc. To see what attributes a group or any other FMLD
object (packages/datasets, organizations, users...) has, you can use
:doc:`FMLD's API </api/index>` to inspect the object. For example to find out what
...
Restart the development web server and reload the `FMLD front page`_ and you
...
.. warning::
   ...
   If a plugin adds a snippet with the same name as one of FMLD's default
...
with the FMLD theme. Let's make them look better by tweaking our template to
use the right HTML tags and CSS classes.

There are two places to look for CSS classes available in FMLD:

1. The `Bootstrap 3.3.7 docs <https://getbootstrap.com/docs/3.3/components/>`_. All of the HTML, CSS and JavaScript
   provided by Bootstrap is available to use in FMLD.

2. FMLD's development primer page, which can be found on any FMLD site at
   ...
   FMLD, and by viewing the source of the page you can see what HTML tags and
...
If you reload the `FMLD front page`_, the most popular groups should look much
better.
...
Reload the `FMLD front page`_, and you should see your activity stream and
...
Not all FMLD config settings are available to templates via
...
If the user sets this config setting to ``True`` in their FMLD config file,
